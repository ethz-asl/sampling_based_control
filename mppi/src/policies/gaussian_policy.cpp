...
Eigen::VectorXd GaussianPolicy::operator()(double t) {
  size_t time_idx = std::distance(
      t_.data(), std::upper_bound(t_.data(), t_.data() + t_.size(), t)) - 1;
  return nominal_.row(time_idx);
}

Eigen::VectorXd GaussianPolicy::operator()(double t, int k) {
  size_t time_idx = std::distance(
      t_.data(), std::upper_bound(t_.data(), t_.data() + t_.size(), t)) - 1;
  return samples_[k].row(time_idx) + nominal_.row(time_idx);
}

void GaussianPolicy::update_samples(const Eigen::VectorXd& weights,
                                    const int keep) {
  if (weights.size() != ns_){
...
void GaussianPolicy::update(const Eigen::VectorXd& weights,
...
void GaussianPolicy::shift(const double t) {
  ...
  if (t < t_[0]){
    std::runtime_error("Shifting back in time!");
  }
