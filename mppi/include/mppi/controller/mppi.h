...
class PathIntegral{
 ...
 private:
  ...
  /**
   * @brief Initializes the variable for multithreading is required (threads > 1)
   */
  void init_threading();
 ...
 public:
  ...
  /**
   * @brief Transforms the cost to go into the corresponding desirability function
   ...
   * @details The previous optimal rollout as well a portion of cached rollouts (according to the caching_factor)
   * get trimmed up to the new reset time. The remaining are cleared to be overwritten in the next control loop.
   * Cached rollouts has different additive noise wrt new updated optimal input, thus this needs to be recomputed
   */
  void prepare_rollouts();

  /**
   * @brief Shift the control vector of one element back for receding horizon control
   ...
   */
  template<typename T>
  void shift_back(std::vector<T>& v_out, const T& fill=T(), const int offset=1);

  /**
   * @brief Applies path integral repeteadly from the first starting condition for multiple times.
   ...
   */
  input_array_t offline_control(const DynamicsBase::observation_t& x, const int subit = 1, const double t=0);
   ...
   */
  void get_input_state(const observation_t& x, observation_t& x_nom, observation_t& u_nom, const double t);

  /**
   * @brief Returns the optimal rollout from the latest updated time when calling set_observation
   * @param x the state trajectory
   * @param u the input trajectory
   * @return false if time is later then the latest available in the nominal rollout
   */
  bool get_optimal_rollout(observation_array_t& x, input_array_t& u);

  // TODO (giuseppe) limit according to actuation limits
  // this requires to recompute noise wrt to previous action and recompute the noise statistics too.
  void bound_action();
  ...
  /**
   * @brief Copy latest observation and time to internal variables used for the next optimization
   * @details Copying latest x and t to internal variables is required to perform the next
   * optimization starting from the same initial state and time while the public one can be
   * potentially changed by a different thread.
   */
  void copy_observation();

  /**
   * @brief Set the reference of the cost function to the latest received for the new optimization
   */
 ...
 public:
  /**
   * @brief Set the reference trajectory to be used in the next optimization loop
 ...
 protected:
  ...
  observation_t x_;                         // utility variable, store the currently simulated step state
  ...
  std::atomic_bool  observation_set_;       // flag to check that observation has ever been set
