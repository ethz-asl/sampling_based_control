...
class PathIntegralVisualizer : public PathIntegral {
 ...
 public:
   ...
   */
   virtual void visualize_single_trajectory(const observation_array_t& traj, double dt=0) = 0;

   /**
    * @brief Same as visualization for single trajectory, specialized on the optimized one
    * @param traj: optimal trajectory rollout
    */
   virtual void visualize_optimal_trajectory(const observation_array_t& traj) = 0;

   /**
    * @brief Self explanatory
    */
   virtual void visualize_all_trajectories(){ std::cout << "Not implemented." << std::endl; };

  /**
   * @brief Print to console the rollouts info such as histogram, cost, min and max cost
   */
 ...
 private:
   ...
   * @param step_count: current step in the simulation (theoretically (int)t/step_size)
   */
  input_t step(const observation_t& x, const double t, bool show, size_t step_count);
