...
class tree {
 ...
 public:
  ...
  /// given iterator. If 'walk_back=true', a depth=0 iterator will be taken from
  /// the beginning of the sibling range, not the current node.
  ...
  /// Return a path (to be taken from the 'top' node) corresponding to a node in
  /// the tree. The first integer in path_t is the number of steps you need to
  /// go 'right' in the sibling chain (so 0 if we go straight to the children).
  ...
  /// Return iterator to the parent of a node. Throws a `navigation_error` if
  ...
  /// node pointed to by position. Does not change the subtree itself (use
  ...
  /// Replace node with a new node, making the old node (plus subtree) a child
  /// of the new node.
  template <typename iter>
  iter wrap(iter position, const T& x);
  /// Replace the range of sibling nodes (plus subtrees), making these children
  /// of the new node.
  template <typename iter>
  ...
  /// Extract the subtree starting at the indicated node, removing it from the
  /// original tree.
  tree move_out(iterator);
  /// Inverse of take_out: inserts the given tree as previous sibling of
  /// indicated node by a move operation, that is, the given tree becomes empty.
  /// Returns iterator to the top node.
  template <typename iter>
  iter move_in(iter, tree&);
  /// As above, but now make the tree the last child of the indicated node.
  template <typename iter>
  iter move_in_below(iter, tree&);
  /// As above, but now make the tree the nth child of the indicated node (if
  /// possible).
  template <typename iter>
  ...
  /// Exchange two nodes (plus subtrees). The iterators will remain valid and
  ...
  /// Compute the depth to the root, counting all levels for which predicate
  /// returns true.
  ...
  template <class Predicate>
  static int distance(const iterator_base& top, const iterator_base& bottom,
                      Predicate p);
  ...
  /// Determine whether node at position is in the subtrees with indicated top
  /// node.
  ...
  /// For debugging only: verify internal consistency by inspecting all pointers
  /// in the tree (which will also trigger a valgrind error in case something
  /// got corrupted).
  ...
  tree_node *head,
      *feet;  // head/feet are always dummy; if an iterator points to them it is
              // invalid
 private:
...
void tree<T, tree_node_allocator>::erase_right_siblings(
  ...
  if (it.node->parent != 0) it.node->parent->last_child = it.node;
}
...
void tree<T, tree_node_allocator>::erase_left_siblings(
  ...
  if (it.node->parent != 0) it.node->parent->first_child = it.node;
}
...
tree<T, tree_node_allocator>::begin(const iterator_base& pos) {
  assert(pos.node != 0);
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::sibling_iterator
tree<T, tree_node_allocator>::end(const iterator_base& pos) {
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::path_t
tree<T, tree_node_allocator>::path_from_iterator(
    const iterator_base& iter, const iterator_base& top) const {
  path_t path;
  tree_node* walk = iter.node;

  do {
    if (path.size() > 0) walk = walk->parent;
    int num = 0;
...
tree<T, tree_node_allocator>::iterator_from_path(
    const path_t& path, const iterator_base& top) const {
  iterator it = top;
  tree_node* walk = it.node;

  for (size_t step = 0; step < path.size(); ++step) {
    if (step > 0) walk = walk->first_child;
    if (walk == 0)
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::pre_order_iterator
tree<T, tree_node_allocator>::set_head(T&& x) {
  assert(head->next_sibling == feet);
  return insert(iterator(feet), x);
}
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::iterator
tree<T, tree_node_allocator>::lowest_common_ancestor(
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::pre_order_iterator&
tree<T, tree_node_allocator>::pre_order_iterator::next_skip_children() {
  (*this).skip_children();
  (*this)++;
  return *this;
}
