 ...
 */
std::vector<double> ComputeWeights(const int m, const int t, const int n,
                                   const int s);
...
struct SavitzkyGolayFilterConfig {
   ...
   * @param dt Time step
   */
  SavitzkyGolayFilterConfig(unsigned m, int t, unsigned n, unsigned s,
                            double dt = 1.)
  ...
  friend std::ostream& operator<<(std::ostream& os,
                                  const SavitzkyGolayFilterConfig& conf);
};
...
struct SavitzkyGolayFilter {
  ...
  explicit SavitzkyGolayFilter(const SavitzkyGolayFilterConfig& conf)
      : conf_(conf) {
    init();
  }

  SavitzkyGolayFilter() { init(); }
  ...
  /**
   * @brief Apply Savitzky-Golay convolution to the data x should have size
   * 2*m+1 As the function only applies a convolution, it runs in O(2m+1), and
  ...
  typename ContainerT::value_type filter(const ContainerT& v) const {
    if (!(v.size() == weights_.size() && v.size())) {
      std::stringstream is;
      is << "data to be filtered have size: " << v.size()
         << " while weights_ size is: " << weights_.size();
