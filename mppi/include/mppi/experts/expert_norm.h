...
class NormExp : public ExpertBase {
 public:
  NormExp(char short_name, config_t config, dynamics_ptr dynamics)
    ...
    expert_sampler_one_.set_covariance(Eigen::VectorXd::Ones(dynamics_->get_input_dimension()) *
                                       0.05);
  ...
  Eigen::VectorXd get_sample(size_t step) override { return expert_sampler_one_.get_sample(); }

  Eigen::MatrixXd get_sigma_inv(size_t step) override { return expert_sampler_one_.sigma_inv(); }
