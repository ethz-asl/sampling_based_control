...
class DoubleIntegratorDynamics : public DynamicsBase {
 public:
  ...
  dynamics_ptr clone() const override { return std::make_shared<DoubleIntegratorDynamics>(*this);}

  void reset(const observation_t &xr) override {
    x = xr;
  }
  ...
  // does not make sense, just to test this function is used properly in the control loop
...
class DoubleIntegratorCost : public CostBase{
 public:
  ...
  cost_t compute_cost(const observation_t& x, const reference_t& r, const double t){
