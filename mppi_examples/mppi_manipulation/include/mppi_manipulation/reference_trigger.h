...
class ReferenceTrigger{
  void add_reference(const double t, const mppi::reference_t& ref){
    if (t < times_.back()){
      std::cout << "[ReferenceTrigger] Trying to add a reference back in the past." << std::endl;
      return;
    }
  ...
  bool parse_from_file(const std::string& file_path){
    ...
    if (!config.IsSequence()){
      std::cout << "[ReferenceTrigger] Failed to parse references file." << std::endl;
      return false;
    }

    for (const auto& element : config){
      if (!element.IsMap()){
        std::cout << "[ReferenceTrigger] Element should be dictionary" << std::endl;
      }
