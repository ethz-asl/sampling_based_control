...
void PandaControllerInterfaceNoRos::set_reference(
    mppi::reference_array_t refs, mppi::time_array_t t
    ){
  mppi::reference_trajectory_t reference_trajectory;
  reference_trajectory.rr = refs;
  reference_trajectory.tt = t;
...
bool PandaControllerInterfaceNoRos::set_controller(mppi::solver_ptr& controller) {
  ...
  // config
  // -------------------------------
  if (!solver_config_.init_from_file(manipulation_config_.solver_config_file)) {
  ...
  if (manipulation_config_.gaussian_policy) {
    policy = std::make_shared<mppi::GaussianPolicy>(
        dynamics->get_input_dimension(), solver_config_);
  } else {
  ...
  // controller
  // -------------------------------
  controller = std::make_shared<mppi::Solver>(dynamics, cost, policy, solver_config_);

  // -------------------------------
  // initialize reference
  // -------------------------------
  ref_.rr.resize(1, mppi::observation_t::Zero(PandaDim::REFERENCE_DIMENSION));
  // init obstacle far away
  ref_.rr[0](7) = 100;
  ref_.rr[0](8) = 100;
  ref_.rr[0](9) = 100;
