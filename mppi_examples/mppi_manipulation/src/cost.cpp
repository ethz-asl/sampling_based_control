...
mppi::CostBase::cost_t PandaCost::compute_cost(const mppi::observation_t& x,
  ...
  if (mode == 0) {
    ...
    cost += (error_.head<3>().transpose() * error_.head<3>()).norm() * param_.Qt;
    cost += (error_.tail<3>().transpose() * error_.tail<3>()).norm() * param_.Qr;
  ...
  // reach the handle with open gripper
    ...
    error_ = mppi_pinocchio::diff(robot_model_.get_pose(tracked_frame_), object_model_.get_pose(handle_frame_) * param_.grasp_offset);
    cost += (error_.head<3>().transpose() * error_.head<3>()).norm() * param_.Qt;
    cost += (error_.tail<3>().transpose() * error_.tail<3>()).norm() * param_.Qr;
  ...
  // move the object
    ...
    object_model_.update_state(x.tail<2 * OBJECT_DIMENSION + CONTACT_STATE>().head<1>());
    error_ = mppi_pinocchio::diff(robot_model_.get_pose(tracked_frame_), object_model_.get_pose(handle_frame_) * param_.grasp_offset);
    cost += (error_.head<3>().transpose() * error_.head<3>()).norm() * param_.Qt2;
    cost += (error_.tail<3>().transpose() * error_.tail<3>()).norm() * param_.Qr2;
