...
int main(int argc, char** argv) {
  ...
  auto cb = [&](const sensor_msgs::JointStateConstPtr & msg) {
    ...
    for (size_t idx =0; idx<msg->name.size(); idx++){
      if(std::find(joint_names.begin(), joint_names.end(), msg->name[idx]) != joint_names.end()){
  ...
  ros::Subscriber joint_sub = nh.subscribe<sensor_msgs::JointState>("/joint_states" ,1, cb);
  ros::spin();
  return 0;
}