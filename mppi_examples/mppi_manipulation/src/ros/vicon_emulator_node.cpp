...
int main(int argc, char** argv){
  ...
  ros::Publisher odometry_publisher = nh.advertise<nav_msgs::Odometry>("/panda_base/vrpn_client/estimated_odometry", 1);
  ...
  auto cb = [&](const gazebo_msgs::LinkStatesConstPtr & msg) {
      ...
      auto iter = std::find_if(msg->name.begin(), msg->name.end(), [&](const std::string& str) {
        return str.find(link_name) != std::string::npos; });
      size_t idx = iter - msg->name.begin();
      if (idx == msg->name.size()){
        ROS_WARN_STREAM_THROTTLE(2.0, "Link: " << *iter << " not found in gazebo link states.");
      }
      else{
  ...
  ros::Subscriber link_subscriber = nh.subscribe<gazebo_msgs::LinkStates>("/gazebo/link_states", 1,
                                                                          cb);
  ros::spin();
  return 0;
}
