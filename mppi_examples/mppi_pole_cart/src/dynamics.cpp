...
void PoleCartDynamics::compute_velocities(double F) {
  xd_(0) = x_(2);
  xd_(1) = x_(3);
  xd_(2) = (F - config_.mux * x_(2) + config_.mp * 9.81 * std::cos(x_(1)) * std::sin(x_(1)) +
      config_.mp * x_(1) * x_(1) * config_.l * std::sin(x_(1))) /
      (config_.mc + config_.mp * std::sin(x_(1)) * std::sin(x_(1)));
  xd_(3) = (-config_.mutheta * x_(3) - 9.81 * std::sin(x_(1)) - xd_(2) * std::cos(x_(1))) / config_.l;
}

void PoleCartDynamics::integrate_internal(double u, double dt) {
  if (dt > config_.dt_internal)
    throw std::runtime_error("Integrate internal called with dt smaller that internal dt");

  compute_velocities(u);
  x_ += xd_ * dt;
}

DynamicsBase::observation_t PoleCartDynamics::step(const DynamicsBase::input_t &u, const double dt) {
