...
void RoyalPandaSim::read_sim(ros::Time time, ros::Duration period) {
  // clang-format off
  mppi::observation_t state = dynamics_->get_state();
...
void RoyalPandaSim::advance_sim(ros::Time time, ros::Duration period) {
  ...
  // arm has already gravity compensation
  Eigen::VectorXd tau = dynamics_->get_panda()->getNonlinearities(dynamics_->get_world()->getGravity()).e();
