...
void msgToEigen_panda(const manipulation_msgs::State& stateRos,
  ...
  for (size_t i = 0 ; i < 7; i ++){
    state(18 + i ) = stateRos.object_state.position[i];
    state(25 + i ) = stateRos.object_state.velocity[i]; 
  }
  state(32) = stateRos.in_contact;
}
...
void eigenToMsg_panda(const Eigen::VectorXd &state, const double &time,
  ...
  for (size_t i = 0 ; i < 7 ; i ++){
    stateRos.object_state.position[i] = state(18 +i);
    stateRos.object_state.velocity[i] = state(25 +i);
  }           
  stateRos.in_contact = state(32);  
}
...
void eigenToMsg_panda(const Eigen::VectorXd& input, manipulation_msgs::Input &inputRos){
  inputRos.joint_velocities.resize(manipulation_msgs::Input::SIZE);
  for (size_t i = 0; i < 8; i++) {
...
void toEigenState_panda(const Eigen::VectorXd& arm_position,
  ...
  state.segment(18,7) << object_position;
  state.segment(25,7) << object_velocity;
  state(32) = contact_state;
}
...
void fromEigenState_panda(Eigen::VectorXd& arm_position,
  ...
  for (int i = 0; i < 9; i++) {
    arm_position(i) = state(i);
    arm_velocity(i) = state(9 + i);
  }

  object_position << state.segment(18,7);
  object_velocity << state.segment(25,7);
  contact_state = state(32);
}
...
void toMsg_panda(const double& time,
  ...
  for (int i  = 0; i < 7; i++)
  {
    stateRos.object_state.position[i] = object_state.position[i];
    stateRos.object_state.velocity[i] = object_state.velocity[i];
  }
  stateRos.in_contact = contact_state;
}
