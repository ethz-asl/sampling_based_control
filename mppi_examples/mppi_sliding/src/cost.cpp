...
mppi::cost_t PandaCost::compute_cost(const mppi::observation_t& x,
  ...
  cylinder_position_ << x(2 * BASE_ARM_GRIPPER_DIM),
      x(2 * BASE_ARM_GRIPPER_DIM + 1), x(2 * BASE_ARM_GRIPPER_DIM + 2);
  // ROS_INFO_STREAM("EE in world frame: " <<
  // robot_model_.get_pose(params_.tracked_frame).translation);
  ...
  if (mode == 0) {
    ...
    // for test dynamic EE pose tracking (object state serves as desired EE
    // state)
  ...
  // handle reaching cost
    ...
    position_dist =
        robot_model_.get_pose(params_.tracked_frame).translation.head<2>() -
        cylinder_position_.head<2>();
    cost += (position_dist.norm() - 0.05) * params_.Qt;
    cost += abs(robot_model_.get_pose(params_.tracked_frame).translation(2) -
                (cylinder_position_(2) - 0.05)) *
            params_.Qt;
  ...
  // object displacement cost
    ...
    pose_diff_1(0) =
        robot_model_.get_pose(params_.tracked_frame).translation(0) -
        cylinder_position_(0);
    pose_diff_1(1) =
        robot_model_.get_pose(params_.tracked_frame).translation(1) -
        cylinder_position_(1);
    pose_diff =
        pose_diff_1(0) * pose_diff_1(0) + pose_diff_1(1) * pose_diff_1(1);
    // pose_diff_1(2) =
    // abs(robot_model_.get_pose(params_.tracked_frame).translation(2)-
  ...
  // // TODO(giuseppe) hard coded for now to match the collision pairs of the
  // safety filter robot_model_.get_offset("panda_link0", "panda_link7",
  // collision_vector_); cost += params_.Q_collision * std::pow(std::max(0.0,
  // params_.collision_threshold - collision_vector_.norm()), 2);
