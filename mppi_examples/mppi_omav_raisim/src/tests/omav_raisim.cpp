/*!
 * @file     omav_raisim.cpp
 * @author   Matthias Studiger
 * @date     23.03.2021
 * @version  1.0
 * @brief    Simulation of the omav dynamics class with raisim backend
 */
...
int main(int argc, char **argv) {
  ros::init(argc, argv, "omav_raisim_sim");
  ros::NodeHandle nh("~");
  ...
  // Instantiate the input vector to hover and slightly turn
  ...
  std::cout << "Initial state: " << simulation.get_state().transpose()
            << std::endl;
  ...
  for (size_t i = 0; i < loopN; i++) {
    ...
    ROS_INFO_STREAM_THROTTLE(1.0, "OMAV state is: " << simulation.get_state());

    raisim::MSLEEP(10);
    ROS_INFO_STREAM_THROTTLE(1.0, "Sim time: " << i * TIMESTEP
                                               << " s, elapsed sim dt: "
                                               << elapsed << " ms.");
  }
