...
bool OMAVControllerInterface::set_controller(std::shared_ptr<mppi::PathIntegral> &controller) {
    // -------------------------------
    // config
    // -------------------------------
    ...
    dynamics = std::make_shared<OMAVRaisimDynamics>(robot_description_raisim, config_.step_size);
    ...
    auto cost = std::make_shared<OMAVRaisimCost>("", cost_param);
