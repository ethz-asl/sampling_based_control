...
OMAVRaisimDynamicsRos::OMAVRaisimDynamicsRos(const ros::NodeHandle &nh,
                                             const double dt)
    : OMAVRaisimDynamics(dt) {
  state_publisher_ =
      nh_.advertise<sensor_msgs::JointState>("/joint_states", 10);

  joint_state_.name = {"omav_pos_x", "omav_pos_y", "omav_pos_z",
                       "omav_rot_x", "omav_rot_y", "omav_rot_z"};
