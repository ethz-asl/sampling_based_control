...
bool PandaMobileControllerInterface::init_ros() {
  ...
  obstacle_subscriber_ = nh_.subscribe("/obstacle", 10, &PandaMobileControllerInterface::obstacle_callback, this);
...
bool PandaMobileControllerInterface::set_controller(std::shared_ptr<mppi::PathIntegral> &controller) {
  // -------------------------------
  // internal model
  // -------------------------------
  init_model();
...
void PandaMobileControllerInterface::ee_pose_desired_callback(const geometry_msgs::PoseStampedConstPtr& msg){
...
void PandaMobileControllerInterface::obstacle_callback(const geometry_msgs::PoseStampedConstPtr& msg){
  std::unique_lock<std::mutex> lock(reference_mutex_);
  obstacle_pose_ = *msg;
