...
PandaMobileDynamics::PandaMobileDynamics(const std::string& robot_description,
                                         const double& dt, bool holonomic)
...
mppi::observation_t PandaMobileDynamics::step(const mppi::input_t& u,
  ...
  if (holonomic_) {
    x_(7) += (vx * std::cos(yaw) - vy * std::sin(yaw)) * get_dt();
    x_(8) += (vx * std::sin(yaw) + vy * std::cos(yaw)) * get_dt();
  } else {
    x_(7) += vx * std::cos(yaw) * get_dt();
    x_(8) += vx * std::sin(yaw) * get_dt();
  }
  x_(9) += yawd * get_dt();
  return x_;
}
