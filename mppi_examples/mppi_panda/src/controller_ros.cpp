...
bool PandaControllerRosBase<StateInterface, StateHandle>::init(hardware_interface::RobotHW* robot_hw, ros::NodeHandle& node_handle, ros::NodeHandle& ctrl_handle) {
  ...
  if (kp.size() != 7){
    ROS_ERROR_STREAM("The proportional gain has the wrong size: " << kp.size() << "!=" << 7);
    return false;
  }
  if (kd.size() != 7){
    ROS_ERROR_STREAM("The proportional gain has the wrong size: " << kp.size() << "!=" << 7);
  ...
  ee_pose_publisher_ = std::make_unique<realtime_tools::RealtimePublisher<geometry_msgs::PoseStamped>>(node_handle, "/end_effector", 10);
  return true;
}

bool PandaControllerRos::addStateHandles(hardware_interface::RobotHW* robot_hw) {
...
bool PandaControllerRosSim::addStateHandles(hardware_interface::RobotHW* robot_hw) {
...
void PandaControllerRosSim::update(const ros::Time& time, const ros::Duration& period) {
...
void PandaControllerRos::update(const ros::Time& time, const ros::Duration& period) {
...
PLUGINLIB_EXPORT_CLASS(panda::PandaControllerRos, controller_interface::ControllerBase)
PLUGINLIB_EXPORT_CLASS(panda::PandaControllerRosSim, controller_interface::ControllerBase)
