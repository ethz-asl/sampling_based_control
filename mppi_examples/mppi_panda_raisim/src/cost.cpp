...
// (video first with emerging behaviour)
...
mppi::CostBase::cost_t PandaCost::compute_cost(const mppi::observation_t& x,
  ...
  // reach the handle with open gripper
    ...
    err_ =
        pinocchio::log6(pose_current_.actInv(pose_handle_.act(grasp_offset_)));
    linear_cost = err_.linear().transpose() * Q_linear_ * err_.linear();
    angular_cost = err_.angular().transpose() * Q_angular_ * err_.angular();
  ...
  // open the door
    ...
    door_opening_cost =
        std::pow(x(2 * PandaDim::JOINT_DIMENSION) - M_PI / 2, 2) * 10;
  }

  double obstacle_dist =
      (pose_current_.translation() - ref.segment<3>(7)).norm();
  if (obstacle_dist < obstacle_radius_) obstacle_cost = Q_obst_;
  ...
  return linear_cost + angular_cost + obstacle_cost + joint_limit_cost +
         door_opening_cost + contact_cost;
}
