...
void to_msg(const mppi::SolverConfig& config, Config& config_ros) {
  ...
  config_ros.input_min.array.assign(config.u_min.data(), config.u_min.data() + config.u_min.size());
...
void to_msg(const mppi::Rollout& rollout, Rollout& rollout_ros) {
  ...
  rollout_ros.cost_vector.array.assign(rollout.cc.data(), rollout.cc.data() + rollout.cc.size());
  ...
  for (size_t i = 0; i < rollout.steps_; i++) {
    state.array.assign(rollout.xx[i].data(), rollout.xx[i].data() + rollout.xx[i].size());
    input.array.assign(rollout.uu[i].data(), rollout.uu[i].data() + rollout.uu[i].size());
    noise.array.assign(rollout.nn[i].data(), rollout.nn[i].data() + rollout.nn[i].size());
...
//  }
  ...
  data_ros.weights.array.assign(data.weights.data(), data.weights.data() + data.weights.size());
}
